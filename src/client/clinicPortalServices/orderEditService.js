...
export const fetchOrderEditData = (editParams) => {
	return fetch(`${serviceConstants.API_HOST_NAME}/order/v2/`, {
		...
		body: JSON.stringify({
			...
			collectedDate:
				editParams && editParams.collectedDate ? editParams.collectedDate : "",
			provider: editParams && editParams.provider ? editParams.provider : "",
			receivedDate:
				editParams && editParams.receivedDate ? editParams.receivedDate : "",
			requisition:
				editParams && editParams.requisition ? editParams.requisition : "",
			patientName:
				editParams && editParams.patientName ? editParams.patientName : "",
		}),
